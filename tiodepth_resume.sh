#!/bin/bash
#SBATCH --job-name=tiodepth_id3852_resume        # 作业名
#SBATCH --output=debug_%j.log        # 输出日志文件，%j是作业ID
#SBATCH --error=dennug_%j.log          # 错误日志文件，%j是作业ID
...
# 运行训练脚本并保存日志
python train_dist_2.py --name TiO-Depth-DINOv2B_rc256_KITTI_S_B8_unfreezeboth_rnoutput_resumeep21 \
    --exp_opts options/TiO-Depth/train/tio_depth-dinov2b_384crop256_kitti_stereo.yaml \
    --batch_size 8 \
    --metric_source rawdepth sdepth refdepth \
    --save_freq 1 \
    --visual_freq 1000 \
    --pretrained_path '/home/ywan0794@acfr.usyd.edu.au/TiO-Depth_pytorch/train_log/2025-04-05_19h16m18s_TiO-Depth-DINOv2B_rc256_KITTI_S_B8_unfreezeboth_rnoutput/model/last_model20.pth' \
