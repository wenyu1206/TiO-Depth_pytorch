#!/bin/bash
#SBATCH --job-name=tiodepth_unfreezeboth_rnoutput          # 作业名
#SBATCH --output=pure_%j.log        # 输出日志文件，%j是作业ID
#SBATCH --error=test_%j.log          # 错误日志文件，%j是作业ID
...
#SBATCH --mem=15G                     # 分配内存大小
#SBATCH --nodelist=persephone             # 指定运行在erinyes节点上
...
# 运行训练脚本并保存日志
python train_dist_2.py --name TiO-Depth-DINOv2B_rc256_KITTI_S_B8_unfreezeboth_rnoutput \
